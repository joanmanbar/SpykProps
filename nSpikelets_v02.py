...
def SpikeProps(Im):
    ...
    # io.imshow(image1)
    image_rescaled = rescale(image1, 0.25, preserve_range=True)
...
#       io.imshow(labeled_spks == 5)
        ...
        orientation = regions[spk-1].orientation
        ...
        Ax = Cx - math.cos(orientation) * 0.5 * regions[spk-1].major_axis_length
        Ay = Cy + math.sin(orientation) * 0.5 * regions[spk-1].major_axis_length
...
#        BranchesPerSpike.to_csv(r'Branches_in_image_v01.csv', header=True)
        ...
        # io.imshow(distance)
        # local_maxi = feature.peak_local_max(distance, indices=False, footprint=morphology.diamond(30), labels=myspk)
        ...
        # local_maxi = feature.corner_peaks(distance, indices=False, min_distance=20, labels=myspk)
        ...
        ax[0].imshow(myspk, cmap=plt.cm.gray)
