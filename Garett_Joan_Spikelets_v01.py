...
#    io.imshow(bw0)  
    ...
    bw1 = morphology.remove_small_objects(bw0, min_size=5000) #
#    io.imshow(bw1)
...
#       io.imshow(labeled_spks == 3)
    ...
    # ------------ SPIKE ROTATION ------------ 
        ...
        # Get bbox's orientation
        current_angle = props_spikes[spk-1].orientation
        
        # Get Centroid coordinates
        Cx = props_spikes[spk-1].centroid[1]
        Cy = props_spikes[spk-1].centroid[0]
        
        # Get point A's coordinates
        Ax = Cx - math.cos(current_angle) * 0.5 * props_spikes[spk-1].major_axis_length
        Ay = Cy + math.sin(current_angle) * 0.5 * props_spikes[spk-1].major_axis_length
        ...
        # minr, minc, maxr, maxc = props_spikes[spk-1].bbox
...
#        io.imshow(skel2)
    ...
    # ------------ MEASURE BRANCHES ------------
        ...
        # Uncomment ONLY if you want to look at the skeleton (degrees)
...
#        BranchesPerSpike.to_csv(r'Branches_in_image_v01.csv', header=True)
    ...
    # ------------ PREPARE DATA TO EXPORT------------         
        ...
        for ind,props in enumerate(props_spikes):
            Spk = props.label
            Area = props.area
    ...
    # ------------ WATERSHED ------------
        ...
        props_spikelets = regionprops(labeled_spikelets)
...
Branches_data.to_csv (r'Images\OUT_DIR\Branches_data_3.csv', header=True, index=False)

# Export Branches_data to csv
Spikes_data.to_csv (r'Images\OUT_DIR\Spikes_data_3.csv', header=True, index=False)
